...
MODULES: Dict[str, Dict[str, Any]] = {
    "customers": {
        "title": "العملاء",
        "keywords": ["عميل", "عملاء", "customer", "زبون", "زبائن", "كشف عميل", "رصيد عميل"],
        "description": "إدارة العملاء وأرصدة العملاء وكشوف الحساب وربطهم بالمبيعات والصيانة والدفعات.",
        ...
        "steps": ["افتح صفحة العملاء", "ابحث أولًا بالاسم أو الهاتف لتجنب التكرار", "اختر إضافة عميل إذا لم يكن موجودًا", "أدخل الاسم والهاتف والبيانات الأساسية", "راجع الرصيد الافتتاحي ولا تدخله إلا بسند", "احفظ ثم افتح كشف الحساب للتأكد من ظهور العميل"],
        "expert_checks": ["افحص التكرار بالهاتف قبل الحفظ", "لا تفسر إشارة الرصيد دون سياسة النظام", "راجع آخر فاتورة وآخر دفعة قبل الحكم على ذمة العميل"],
        "after_save": ["افتح كشف العميل", "اربط العميل بالمركبة أو طلب الصيانة إن وجد", "راجع الدفعات القديمة أو الرصيد الافتتاحي"],
        "common_mistakes": ["إضافة نفس العميل باسم مختلف", "إدخال رصيد افتتاحي بلا مرجع", "اعتبار الموجب/السالب عليه أو له دون معرفة سياسة النظام"],
    ...
    "suppliers": {
        ...
        "expert_checks": ["طابق اسم المورد مع فواتير الشراء", "لا تجمع المورد والعميل في حساب واحد إلا إذا النظام يدعم ذلك", "راجع التسويات قبل إصدار حكم على الرصيد"],
    ...
    "sales": {
        ...
        "steps": ["اختر العميل", "اختر المستودع", "أضف المنتجات والكميات", "راجع توفر الكمية وسعر كل بند", "راجع الخصم والضريبة من إعدادات النظام", "احفظ الفاتورة", "سجل الدفعة إن حصل قبض", "اطبع أو أرسل الفاتورة عند الحاجة"],
        "expert_checks": ["تأكد من المخزون قبل الحفظ", "راجع العميل والعملة وطريقة الدفع", "لا تعتمد VAT إلا من إعدادات النظام", "راجع أثر الفاتورة على الذمة والمخزون"],
        "after_save": ["افتح الفاتورة وتحقق من الإجمالي", "راجع حركة المخزون", "راجع كشف العميل", "سجل الدفعة أو اربطها بالفاتورة"],
    ...
    "payments": {
        ...
        "description": "إدارة الدفعات الواردة والصادرة وربطها بالعملاء أو الموردين أو الفواتير.",
        ...
        "steps": ["حدد هل العملية قبض IN أو صرف OUT", "اختر الجهة: عميل/مورد/شريك/مصروف حسب النظام", "أدخل المبلغ وطريقة الدفع", "اربط الدفعة بالفاتورة إن وجدت", "أضف المرجع أو رقم الشيك", "احفظ وراجع كشف الجهة"],
    ...
    "services": {
        ...
        "fields": {"customer_id": "العميل", "vehicle_model": "موديل المركبة", "vehicle_vrn": "رقم المركبة", "problem_description": "وصف العطل", "mechanic_id": "المسؤول", "estimated_cost": "التكلفة المتوقعة"},
        "steps": ["اختر العميل أو أضفه إن لم يكن موجودًا", "أدخل بيانات المركبة ورقمها", "اكتب وصف العطل كما قال العميل", "عيّن المسؤول أو الفني", "أضف التشخيص والقطع المستخدمة", "راجع التكلفة والدفعات", "غيّر الحالة عند اكتمال العمل"],
        "expert_checks": ["وصف العطل الأولي غير التشخيص النهائي", "قطع الغيار يجب أن تخصم من المخزون إذا النظام يدعم ذلك", "لا تغلق الطلب قبل مراجعة التكلفة والقبض", "اربط الدفعات بطلب الصيانة أو العميل"],
        "after_save": ["تابع الحالة", "أضف قطع الغيار", "راجع فاتورة/تحصيل العميل", "اطبع تقرير أو وصل"],
        "common_mistakes": ["نسيان ربط العميل", "إغلاق الطلب قبل التحصيل", "استخدام قطع دون خصم مخزون"],
...
class UserGuideMaster:
    ...
    def _professional_hint(self, key: str, action: str) -> str:
        hints = {
            "customers": "المستخدم المتمرس يبدأ بالبحث عن العميل ثم يراجع كشفه قبل إنشاء حركة جديدة.",
            ...
            "sales": "المستخدم المتمرس يراجع العميل، المستودع، الكمية، الضريبة، ثم الدفعة والرصيد بعد الحفظ.",
    ...
    def _general_help(self) -> Dict[str, Any]:
        return {
            ...
            "message": "اسألني عن العملاء، الموردين، المنتجات، المبيعات، الدفعات، المخازن، المصروفات، الصيانة، التقارير، أو دفتر الأستاذ.",
